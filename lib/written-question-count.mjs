...
export function questionBoundsFromConfig(cfg = {}) {
  return normalizeWrittenQuestionBounds(
    ...
    cfg.written_questions_max ?? cfg.max_questions_bound
  );
}

/** Per-session written count — always clamped to admin bounds on session config. */
export function resolveEffectiveWrittenMaxQuestions(cfg = {}, sessionRow = {}) {
