...
export class GeminiLiveBridge {
  ...
  emitAwaitingAnswer(qNum, questionText, opts = {}) {
    ...
    if (isWarmup && ev.warmup) this.warmupAnswerLatched = ev.warmup;
    this.onEvent(ev);
  ...
  openTtsAnswerWindow(qNum, questionText) {
    ...
    if (bytes <= 0) {
      console.warn(`[relay] blocked mic open for Q${qNum} — no TTS audio`);
      this.ttsTurnCompleteReceived = false;
      this.clientPlaybackIdleForQ = 0;
  ...
  handleQuestionAudioMissing(qNum) {
    ...
    this.ttsTurnCompleteReceived = false;
    this.clientPlaybackIdleForQ = 0;
    this.retryQuestionSpeak(q, questionText, { flushFirst: true, directOnly: true });
  }
  ...
  retryWarmupSpeak(phase) {
    ...
    if (this.warmupAnswerLatched === phase) return;
    const wNum = phase === 'mic_check' ? -1 : 0;
    if (this.answerPromptOpen && this.answerPromptFor === wNum) return;
    if (!this.canSendTtsPromptNow()) {
      setTimeout(() => this.retryWarmupSpeak(phase), 800);
      return;
    }
    const key = phase === 'mic_check' ? 'mic_check' : 'intro';
    this.warmupSpeakRetries[key] = (this.warmupSpeakRetries[key] || 0) + 1;
    if (this.warmupSpeakRetries[key] > 2) {
      console.warn(`[relay] warmup ${phase} speak retries exhausted — opening mic without TTS`);
      this.emitWarmupFallback(phase);
      return;
    }
    console.warn(`[relay] warmup ${phase} had no audible output — retry #${this.warmupSpeakRetries[key]}`);
    this.speakWarmupDeterministic(phase, { flushFirst: true });
  }
  ...
  speakWarmupDeterministic(phase, opts = {}) {
    if (this.interviewEnded || this.closed) return;
    ...
    this.sendSpokenPrompt(this.buildWarmupSpeechPrompt(displayText), {
  ...
  speakQuestion(qNum, opts = {}) {
    ...
    if (speakOpts.directOnly) {
      delete speakOpts.prefaceReliable;
      delete speakOpts.prefaceAppreciation;
    }
    this.blockModelOutput = false;
    this.allowModelAudio = true;
  ...
  deliverQuestionTts(qNum, text, opts = {}) {
    ...
    this.questionAudioSafety = setTimeout(() => {
      ...
      if (bytes < minBytes || !this.ttsTurnCompleteReceived) {
        if (bytes > 0 && this.ttsTurnCompleteReceived) {
          this.tryOpenTtsAnswerWindow(qNum);
          return;
        }
        console.warn(`[relay] question ${qNum} TTS safety — re-speaking (bytes=${bytes})`);
        this.queueRetryQuestionSpeak(qNum, text, { flushFirst: true, directOnly: true }, 500);
      } else {
        this.tryOpenTtsAnswerWindow(qNum);
      }
    }, 25000);
  ...
  retryQuestionSpeak(qNum, text, opts = {}) {
    ...
    this.ttsRetryTimer = setTimeout(() => {
      ...
      this.ttsForQ = qNum;
      const speakOpts = {
        directOnly: !opts.prefaceRepeat && !opts.prefaceEnglishNudge && !opts.prefaceIrrelevant,
        ...(this.ttsSpeakOpts || {}),
        ...opts,
        ...(opts.prefaceRepeat || retries >= 4 ? { prefaceRepeat: true, directOnly: false } : {}),
      };
      if (speakOpts.directOnly) {
        delete speakOpts.prefaceReliable;
        delete speakOpts.prefaceAppreciation;
  ...
  proceedAfterAnswer(aNum) {
    ...
    const nextQ = aNum + 1;
    const speakOpts = { directOnly: true };
  ...
  sendClientText(text, turnComplete = true) {
    ...
    } else if (this.warmupTtsOnly && this.warmupTtsPhase) {
      this.markTtsPromptSent();
    } else {
      this.modelAudioThisTurn = false;
    }
